...
extern "C" int pqcrystals_kyber512_90s_ref_keypair_p()
{
    int ret = pqcrystals_kyber512_90s_ref_keypair(pk, sk);
...
extern "C" int pqcrystals_kyber512_90s_ref_enc_p(char const *text, char const *key)
{
    int ret = 0;
    ...
    ret = pqcrystals_kyber512_90s_ref_enc(ct, skey, pk);
...
extern "C" int pqcrystals_kyber512_90s_ref_dec_p(char const *text, char const *key)
{
    int ret = 0;

    ret = decode_base58(reinterpret_cast<unsigned char const*>(text), strlen(text), ct);
    ret = decode_base58(reinterpret_cast<unsigned char const*>(key), strlen(key), sk);
    ret = pqcrystals_kyber512_90s_ref_dec(skey, ct, sk);
    ret = encode_base58(skey, KYBER_SSBYTES, skey_base58, KYBER_SSBYTES * 2);
    skey_ready = 1;
...
PYBIND11_MODULE(pykyber_90s, m) {
  ...
  m.def("pqcrystals_kyber512_90s_ref_keypair", &pqcrystals_kyber512_90s_ref_keypair_p, R"doc(
        512 bit version of Kyber

        Key pair generator.
    )doc");

  m.def("pqcrystals_kyber512_90s_ref_enc", &pqcrystals_kyber512_90s_ref_enc_p, R"doc(
        512 bit version of Kyber

        Encryptor
    )doc");

  m.def("pqcrystals_kyber512_90s_ref_dec", &pqcrystals_kyber512_90s_ref_dec_p, R"doc(
        512 bit version of Kyber
